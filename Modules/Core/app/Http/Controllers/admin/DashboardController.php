...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        // Get visitor count from Redis or fallback to DB
        $pengunjung = null;
        try {
            $pengunjung = \Illuminate\Support\Facades\Redis::get('visitor_count');
        } catch (\Throwable $e) {
        ...
        if (is_null($pengunjung)) {
            ...
            try {
                \Illuminate\Support\Facades\Redis::set('visitor_count', $pengunjung);
            } catch (\Throwable $e) {
                //
            }
        }
        ...
        // Optimized browser & device aggregation selecting only user_agent, cached for 5 minutes
        $visitor_devices = \Illuminate\Support\Facades\Cache::remember('visitor_devices_stats', 300, function() {
            $userAgents = \Modules\Core\Models\Visitor::select('user_agent')->get();
        ...
        // Cached Telemetry Check (cached for 5 minutes)
        $telemetry = \Illuminate\Support\Facades\Cache::remember('telemetry_data', 300, function() {
            $disk_path = base_path();
